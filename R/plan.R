...
plan_doc <- function(sections, level_add = 0, ...) {
  ...
  #env_doc <- rlang::current_env()
  #purrr::imap(s_plans, \(s_plan, sec) eval(s_plan, envir = env_doc))
  result_doc <- list()
...
plan_sec <- function(head_level = NULL, ...) {
  ...
  #env_sec <- rlang::current_env()
  #purrr::imap(b_plans, \(b_plan, block) eval(b_plan, envir = env_sec))
  result_sec <- list()
